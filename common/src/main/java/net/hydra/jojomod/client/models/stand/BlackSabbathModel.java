...
public class BlackSabbathModel<T extends BlackSabbathEntity> extends StandModel<T> {
    ...
    public static LayerDefinition getTexturedModelData() {
        ...
        PartDefinition head = stand2.addOrReplaceChild("head", CubeListBuilder.create(), PartPose.offset(0.0F, -24.15F, -0.75F));
	...
	public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        if (SU != null) {
            if (SU.roundabout$getStandPowers() instanceof PowersBlackSabbath PBS) {
                float tickDelta = mc.getDeltaFrameTime();
                float tickDelta2 = Math.min(tickDelta, 2);
                float r = pEntity.getUser().getViewXRot(tickDelta2);
                float r2 = pEntity.getViewYRot(tickDelta2);
                float rotX = ((r % 360) - swimRotCorrect) * Mth.DEG_TO_RAD;
                float rotX2 = ((r2 % 360)) * Mth.DEG_TO_RAD;
